...
parser = argparse.ArgumentParser(description='Script to create data matrices for input for PGLS/ANOVA')
...
for index in exp_mat.iterrows():
	print index[0]
	if args.lg2:
		outfile = open('phylode_dat/'+str(index[0])+'.dat.log2.csv','w')
	else:
		outfile = open('phylode_dat/'+str(index[0])+'.dat.csv','w')
	...
	for sp in sp_list:
		if args.lg2:
			...
			outfile.write(','.join([sp,state_dict[sp],str(mymean+0.001),str(myse+0.000000001)+'\n']))
		else:
			...
			outfile.write(','.join([sp,state_dict[sp],str(mymean),str(myse)+'\n']))