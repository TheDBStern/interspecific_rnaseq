...
'''
This script takes output from a pairwise (for now) OMA run, extracts those orthologs from the respective 
...
for record in SeqIO.parse(first_fasta, "fasta"):
    for x in genes_first:
        if x == record.id and x not in new_first:
            SeqIO.write(record, outfile_first, "fasta")
            new_first.append(x)
    
for record in SeqIO.parse(second_fasta, "fasta"):
    for x in genes_second:
        if x == record.id and x not in new_second:
#            record.id = ''
            record.description = ortho_second[x]
